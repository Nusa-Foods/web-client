...
export async function generateMetadata(
    ...
    const recipe: RecipeType = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/nusa/${params.slug}`,
        {
            cache: "no-store",
            credentials: "include",
        }
                    ...
                    <div className="flex mb-6 pt-8 px-4 justify-between">
                        <div className="flex flex-row items-center gap-4">
                            <img
                                ...
                                className="w-8 h-8"
                            />
                            <h1 className="text-3xl font-bold">Detail Resep</h1>
                        </div>
                        <div className="flex">
                            <ProfilDropdown />
                        </div>
                        ...
                        {recipe && (
                            <div>
                                <img
                                    src={recipe.imgUrl}
                                    alt={recipe.title}
                                    className="w-full h-auto mb-4 rounded-t-md"
                                />
                                <div className="mb-4">
